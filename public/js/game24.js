const getNumbers = (($) => {
  ...
  return () => {
    return nums.map((n) =>
      n.value <= 0 || n.value > 9 ? alert('Wrong numbers.') : n.value,
    );
  };
...
document.getElementById('solveBtn').addEventListener('click', (e) => {
  ...
  setTimeout(() => {
    if (solution) {
      // equation.textContent = solution + ' = ' + targetNum;
      equation.textContent = 'YES';
    } else {
      // equation.textContent = "It's not possible!";
      equation.textContent = 'NO';
    }
  }, 250);
});
